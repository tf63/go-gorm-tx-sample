...
import (
	...
	"github.com/tf63/go-gorm-tx-sample/internal/context-pattern/application"
	"github.com/tf63/go-gorm-tx-sample/internal/context-pattern/infrastracture"
...
func main() {
	...
	// DI
	txManager := infrastracture.NewTransactionManager(db)
	ar := infrastracture.NewAccountRepository(db)
	au := application.NewAccountUsecase(ar, txManager)
